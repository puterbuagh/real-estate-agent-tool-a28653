...
- Pages: Dashboard (`/`), Property Comparator (2–5 addresses, parallel lookup, save/clear/print), Pipeline (kanban with summary bar + inline add form + DnD across stages), Market Stats (server-fetched live 30yr rate prefill + local inputs + mortgage calc + affordability), Client Report (`/client-report`, saved-comparison dropdown + agent branding + printable preview), Email Report (stub)
